...
function PublisherPage() {
  ...
  const [gameQuery, setGameQuery] = useState<GameQuery>({ publishers: publisherSlug });
  
  useEffect(() => {
    setGameQuery(prev => ({ ...prev, publishers: publisherSlug }));
  }, [publisherSlug]);
  ...
  const displayName = publisherSlug ? publisherSlug.replace(/-/g, ' ') : 'Publisher';
  ...
  return (
    <>
      <SEO 
        ...
        description={`Explore the full catalogue of games published by ${displayName}.`}
      />
      ...
      <div className="grow flex flex-col w-full min-w-0">
        <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-medieval font-bold text-white leading-tight capitalize">
              {displayName}
            </h1>
            <p className="text-zinc-500 text-sm mt-1">
              {searchText ? `Results for "${searchText}"` : 'Explore the full catalogue'}
            </p>
          </div>
      ...
      >
        <span className="text-zinc-400 font-sans text-xs whitespace-nowrap">
          Viewing <span className="font-medieval text-accent tracking-widest uppercase font-bold ml-1">{displayName}</span>
        </span>
        <button
